...
public class clsStudentAccess {
    ...
    public clsStudent LoginEstudianteE(String EmailEstudiante) {
        ...
        try {
            ...
            ps=aux.prepareStatement(sql);
            ps.setString(1, EmailEstudiante);
            rs=ps.executeQuery();
            while(rs.next()){
     ...
     public clsStudent LoginEstudianteC(String ContraEstudiante) {
        
        aux=conexion.conectar();
        clsStudent EstudianteC=null;
        try {
             
            String sql="SELECT nombre, fechanacimiento, tipodocumento, numdocumento, codigo, carrera, email, contraseña FROM student WHERE contraseña=?";
            ps=aux.prepareStatement(sql);
            ps.setString(1, ContraEstudiante);
            rs=ps.executeQuery();
            while(rs.next()){
                EstudianteC = new clsStudent(
                        rs.getString("nombre"),
                        rs.getString("tipodocumento"),
                        rs.getString("carrera"),
                        rs.getString("email"),
