...
public class clsTeacherAccess {
    ...
    public void updateTeacher(clsDocente c, Long codTeacher) {
     ...
     public clsDocente LoginDocenteE(String Email) {
        aux=conexion.conectar();
    ...
    public clsDocente LoginDocenteC(String Contraseña) {
        aux=conexion.conectar();
        clsDocente teacherC=null;
        try {
            String sql="SELECT nombre, fechanacimiento, documento, codigo, email, contraseña FROM teacher WHERE contraseña=?";
            ps=aux.prepareStatement(sql);
            ps.setString(1, Contraseña);
            rs=ps.executeQuery();
            while(rs.next()){
                teacherC = new clsDocente(
                        rs.getString("nombre"),
                        rs.getLong("documento"),
                        rs.getLong("codigo"),
                        rs.getString("email"),
